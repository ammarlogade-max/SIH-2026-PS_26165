...
import { CampbellGateEvaluation } from "@/lib/types";

interface CampbellDecisionTreeProps {
  evaluation?: CampbellGateEvaluation | null;
  compact?: boolean;
}

export default function CampbellDecisionTree({
  evaluation,
  compact = false,
}: CampbellDecisionTreeProps) {
    ...
    return (
      <div className="rounded-lg border border-dashed border-surface-border bg-surface/30 p-4 text-center">
        ...
        <p className="mt-2 text-xs font-semibold text-slate-300">
          Campbell 3-Gate SIF Decision Tree
        </p>
        <p className="mt-1 text-[11px] text-slate-400">
          Run observation through Layer A/B classifier to evaluate the 3 diagnostic gates.
        </p>
      </div>
    );
  }
  ...
  const isPrecursor = decision_verdict === "SIF Precursor";
  const isActual = decision_verdict === "Actual SIF / Major Event";
    ...
    <div className="rounded-lg border border-surface-border bg-surface-card p-4 transition-all">
      ...
      <div className="mt-3 flex items-center justify-between text-[11px] text-slate-400">
        <span className="flex items-center gap-1">
          ...
          SIF Precursor is verified only if High Energy + Compromised Direct Barrier + Line of Fire intersect.
        </span>
        <span className="font-mono text-[10px] text-slate-400">
          Audit Model: Campbell-3G-v2
        </span>
      </div>
    </div>
  );
