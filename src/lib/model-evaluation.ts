 ...
 * Zero simulation or stand-in logic. Every prediction is computed by calling classifyReportLayerA().
 */
export function calculateHeldOutMetrics(): EvaluationReport {
  ...
  for (const item of TEST_SPLIT) {
    ...
    const prob = result.is_sif_potential ? result.confidence / 100 : (100 - result.confidence) / 100;
  ...
  // Clamp AUC between 0.5 and 1.0
  roc_auc = Math.max(0.5, Math.min(1.0, roc_auc));
